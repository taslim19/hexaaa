...
async def send_hunt_command():
    ...
    try:
        ...
        print(f"Error during hunt command: {e}")
        await zzz(3, 5)
...
async def hunt(event):
    ...
    if hunt:
        ...
        if "Shiny" in text:
            await bot.send_message(private_chat_id, "✨ Shiny Pokémon found! @dragtf")
            bot.disconnect()
        elif "TM" in text:
            ...
            await zzz(randint(3, 5))
            await send_hunt_command()
        elif "You caught a wild" in text:  # Check for caught Pokémon
            for pokemon in pokemon_list:
                if pokemon in text:
                    await bot.send_message(private_chat_id, f"🎉 Caught {pokemon}! {text}")
